-- ============================================================
-- English descriptions (grammar/usage notes)
--
-- The `description` field (grammar notes, synonyms, usage hints) was
-- only ever in Russian, even when the interface language was English.
-- This adds description_en, following the same pattern as
-- translation_en / group_en / example_en: optional, falls back to
-- description when blank.
--
-- Schema only — see backfill_description_en.sql for a one-time script
-- that fills in description_en for cards uploaded before this column
-- existed (all 1163 unique existing description values, translated).
-- ============================================================

alter table public.cards
  add column description_en text not null default '';

-- Recreate the view. Same rule as 0002/0003/0004/0005: CREATE OR REPLACE
-- VIEW can only append columns at the very end.
...
-- to carry description_en along too.
