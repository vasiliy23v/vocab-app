...
export default function InvitePage() {
  ...
  const handleAccept = async () => {
    ...
    if (error) {
      ...
      setStatus("done")
      toast.success(t("invite.doneStudentAdded"))
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        <Card className="max-w-sm">
          <CardHeader>
            ...
            <CardDescription>{t("invite.needAuthDesc")}</CardDescription>
          </CardHeader>
          <CardContent>
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <Card className="max-w-sm">
        <CardHeader>
          <CardTitle>{t("invite.title")}</CardTitle>
          <CardDescription>
            {t("invite.codeLabelPrefix")} <code className="font-semibold">{code}</code>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {status === "error" && <p className="text-sm text-destructive">{message}</p>}
          {status === "done" ? (
            <p className="text-sm text-success">{t("invite.acceptedRedirecting")}</p>
          ) : (
            <Button className="w-full" onClick={handleAccept} disabled={status === "working"}>
              {status === "working" ? t("invite.accepting") : t("invite.acceptInvite")}
            </Button>
          )}
        </CardContent>
      </Card>
